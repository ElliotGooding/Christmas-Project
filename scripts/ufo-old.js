...
export class UfoSpawner{
    ...
    spawn(){
        const center = [11.5,25,-2121]
        const dims = [16000, 15, 13000]
        let pos = null;
...
export class Ufo{
    constructor(position, scene, character, renderer, camera, structureObjects){
        ...
        this.equilibrium = [this.position[0], this.y, this.position[2]]
        this.state = "hover"
    ...
    updateState(){
        if (this.state == 'hover'){
            ...
            const characterVec = new THREE.Vector3(this.character.x, this.character.y, this.character.z);
            if (distToPlayer < this.passiveDist && this.checkCollisionBetweenPoints(ufoVec, characterVec)){
                this.state = 'attack'
                this.setTarget();
            }
    ...
    shoot(){
        ...
        if (this.character.admin === false && this.character.dead === false && this.t > 750 && Math.random() < this.shootFreq*this.ticksSinceLastShot/100000){
    ...
    checkAttack(){
        const ufoVec = new THREE.Vector3(...this.position);
        const characterVec = new THREE.Vector3(this.character.x, this.character.y, this.character.z);  
        if (!this.checkCollisionBetweenPoints(ufoVec, characterVec)){
            this.state = 'hover';
            return;
        }
        const distToCharacter = this.getDistToPlayer();
        if (distToCharacter < this.minDist || distToCharacter > this.maxDist){
            this.setTarget();
        } else {
            this.position[0] = this.equilibrium[0] + this.bobAmount*Math.cos(this.t/200);
            this.position[1] = this.equilibrium[1] + this.bobAmount*Math.sin(this.t*2/200)/2;
    ...
    moveTo(){
        ...
        const distToCharacter = (
            (this.character.x - this.target[0])**2 +
            (this.character.z - this.target[2])**2
        )**0.5
        if (distToCharacter < this.minDist || distToCharacter > this.maxDist){
            this.setTarget();
        }
